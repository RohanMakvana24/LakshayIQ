...
import { GraduationCap, LogOut, Search, Bell, ChevronRight } from "lucide-react";
                ...
                <div key={item.label}>
                  <button
                    ...
                    className={cn(
                      ...
                      anyActive ? "text-foreground" : "text-sidebar-foreground hover:bg-sidebar-accent/60"
                    )}
                  >
                  ...
                  {isOpen && (
                    <div className="ml-6 mt-1 space-y-1 border-l pl-3">
                      {item.children.map((c) => {
                        ...
                        return (
                          <Link key={c.to} to={c.to as never} className={cn(
                            "block rounded-md px-3 py-1.5 text-sm transition-colors",
                            active ? "bg-primary/10 font-medium text-primary" : "text-muted-foreground hover:bg-sidebar-accent/60 hover:text-foreground"
                          )}>{c.label}</Link>
                        );
                      })}
                    </div>
        ...
        <div className="border-t p-3">
          <div className="mb-2 flex items-center gap-3 rounded-lg px-2 py-2">
            ...
            <div className="min-w-0 flex-1">
              <p className="truncate text-sm font-medium">{user?.user_metadata?.full_name ?? (variant === "admin" ? "Admin" : "Student")}</p>
