    ...
    <div className="space-y-8 p-1 antialiased animate-fade-in">
      ...
      <header className="relative rounded-3xl border border-neutral-200/60 bg-gradient-to-br from-neutral-900 via-neutral-950 to-neutral-900 p-6 md:p-8 text-white shadow-[0_15px_40px_rgba(0,0,0,0.12)] overflow-hidden group">
        ...
        <div className="relative z-10 space-y-3">
          ...
          <div className="pt-2 flex flex-wrap items-center gap-4 text-xs font-mono text-neutral-300">
            ...
            <span className="flex items-center gap-1.5 bg-black/30 border border-white/5 px-2.5 py-1 rounded-lg backdrop-blur-sm">
              <GraduationCap className="h-3.5 w-3.5 text-neutral-400" /> {course.total_semesters} Semesters Structured
            </span>
          </div>
        </div>
      </header>
      ...
      <section className="space-y-5">
        <div className="border-b border-neutral-100 pb-3">
          <h2 className="font-display text-xl font-bold tracking-tight text-neutral-900">Academic Semesters</h2>
            ...
            {semesters.map((s) => {
              ...
              return (
                <Link key={s.id} to="/student/semester/$id" params={{ id: s.id }} className="block group">
                  <Card className="h-full border border-neutral-200/80 bg-white p-5 rounded-2xl shadow-[0_4px_15px_-3px_rgba(0,0,0,0.01)] transition-all duration-300 hover:-translate-y-1 hover:border-neutral-900 hover:shadow-[0_12px_25px_-6px_rgba(0,0,0,0.05)] flex items-center gap-4 overflow-hidden relative">
                    
                    {/* Dark Minimalistic Index Block */}
                    <div className="grid h-14 w-14 shrink-0 place-items-center rounded-xl bg-neutral-900 text-xl font-bold font-mono text-white shadow-sm transition-transform duration-300 group-hover:scale-105">
                      {s.semester_number}
                    </div>
