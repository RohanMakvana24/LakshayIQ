...
import { Camera, Check, Loader2, User, Mail, Calendar, Shield, AlertCircle, Eye, EyeOff, Lock, Sparkles, LogOut, ShieldAlert } from "lucide-react";
...
function AdminProfilePage() {
  const { user, profile, refreshProfile, role, signOut } = useAuth();
  ...
  const displayName = fullName || profile?.full_name || "Admin Profile";
  ...
  return (
    <div className="w-full py-2 space-y-6 animate-in fade-in duration-300 text-left">
      ...
      <div className="relative rounded-2xl border border-border/80 bg-card overflow-hidden shadow-sm">
        {/* Cover Banner */}
        <div className="relative h-32 md:h-44 w-full bg-gradient-to-r from-slate-950 via-slate-900 to-violet-950">
        ...
        <div className="px-6 pb-6 pt-16 relative flex flex-col md:flex-row md:items-end justify-between gap-4">
          ...
          <div className="absolute -top-12 left-6 h-24 w-24 rounded-2xl overflow-hidden bg-card border-4 border-card shadow-md flex items-center justify-center group">
            {profile?.avatar_url ? (
              ...
              <div className="h-full w-full bg-gradient-to-br from-violet-700 to-indigo-800 text-white text-3xl font-extrabold flex items-center justify-center">
                {initials}
              </div>
            )}
          ...
          <div className="space-y-1">
            <h2 className="text-xl font-black text-foreground flex items-center gap-2">
              {displayName}
              <span className="inline-flex items-center gap-1 rounded-full bg-violet-500/10 border border-violet-500/15 px-2 py-0.5 text-[9px] font-black uppercase text-violet-600 dark:text-violet-400 tracking-wider">
                System Admin
              </span>
            </h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
        ...
        {/* Left column: Admin Status Card & Signout */}
          ...
          <div className="bg-card border border-border/80 rounded-2xl p-5 shadow-sm space-y-4">
            ...
            <div className="space-y-3">
              ...
              <div className="bg-secondary/40 border border-border/40 p-3.5 rounded-xl flex items-center gap-3">
                ...
                <div>
                  <span className="block text-[9px] font-bold text-muted-foreground uppercase tracking-widest leading-none">System Level</span>
            ...
            <div className="pt-2 border-t border-border/40">
              <Button
                ...
                className="w-full h-10 rounded-xl text-xs font-bold uppercase tracking-wider transition-all flex items-center justify-center gap-2 cursor-pointer shadow-sm"
              >
                ...
                <div className="flex flex-col sm:flex-row gap-3">
                  <div className="relative flex-1">
                    ...
                    <Input
                      ...
                      className="pl-10 h-10 rounded-xl border-border focus-visible:ring-violet-500/20 focus-visible:border-violet-500 text-sm font-bold bg-secondary/35"
                    />
                  </div>
                  <Button
                    type="submit"
                    disabled={savingName || fullName === profile?.full_name}
                    className="h-10 px-5 rounded-xl bg-violet-600 hover:bg-violet-700 text-white font-extrabold text-xs uppercase tracking-wider transition-all shrink-0 shadow-sm"
                  >
              ...
              <form onSubmit={handlePasswordChange} className="space-y-4">
                <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    ...
                    <Input
                      ...
                      placeholder="••••••••"
                      required
                      className="pl-10 pr-10 h-10 rounded-xl border-border focus-visible:ring-violet-500/20 focus-visible:border-violet-500 text-sm font-bold bg-secondary/35"
                    />
                    <button
                      type="button"
                ...
                <div className="pt-2 flex justify-end">
                  <Button
                    ...
                    className="h-10 px-6 rounded-xl bg-violet-600 hover:bg-violet-700 text-white font-extrabold text-xs uppercase tracking-wider transition-all shadow-sm flex items-center gap-2"
                  >
                    {updatingPassword ? (
                      <>
